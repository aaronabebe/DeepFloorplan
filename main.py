...
from utils.rgb_ind_convertor import ind2rgb, floorplan_boundary_map, floorplan_fuse_map
...
class MODEL(Network):
    ...
    def infer(self, save_dir='out', resize_img=True, merge=True):
        ...
        if tf.train.latest_checkpoint(self.log_dir) is not None:
        ...
        for p in paths:
            ...
            imwrite(save_path1, out1_rgb)
            imwrite(save_path2, out2_rgb)
            if merge:
                imwrite(save_path3, out3_rgb)
    ...
    def evaluate(self, sess, epoch, num_of_classes=11):
        ...
        file.write('Model at epoch {}: overall accuracy = {:.4}, mean_acc = {:.4}'.format(epoch, overall_acc,
                                                                                          mean_acc9))
        for i in range(mean_acc.shape[0]):
            if i not in [7, 8]:  # ingore class 7 & 8
                file.write('\t\tepoch {}: {}th label: accuracy = {:.4}'.format(epoch, i, mean_acc[i]))
        file.close()
