...
from rgb_ind_convertor import rgb2ind
...
def load_raw_images(path):
    ...
    wall = resize(wall, (512, 512))
    close = resize(close, (512, 512))
    close_wall = resize(close_wall, (512, 512))
    room = resize(room, (512, 512, 3))
...
def read_record(data_path, batch_size=1, size=512):
    ...
    filename_queue = tf.train.string_input_producer([data_path], num_epochs=None, shuffle=False,
...
def load_seg_raw_images(path):
    ...
    # NOTE: imresize will rescale the image to range [0, 255], also cast data into uint8 or uint32
    image = resize(image, (512, 512, 3))
    close = resize(close, (512, 512)) / 255
    close_wall = resize(close_wall, (512, 512)) / 255
    room = resize(room, (512, 512, 3))
...
def read_seg_record(data_path, batch_size=1, size=512):
    ...
    images, labels = tf.train.shuffle_batch([image, label_one_hot],
                                            batch_size=batch_size, capacity=batch_size * 128, num_threads=1,
...
def read_bd_rm_record(data_path, batch_size=1, size=512):
    ...
    # Creates batches by randomly shuffling tensors
    images, label_boundaries, label_rooms, label_doors = tf.train.shuffle_batch(
    ...
    return {'images': images, 'label_boundaries': label_boundaries, 'label_rooms': label_rooms,
